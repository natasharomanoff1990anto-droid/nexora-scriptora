...
const Index = () => {
  ...
  const applyLaunchIntent = (action?: string | null, section?: string | null) => {
    if (section === "publish") setShowPublish(true);
    if (section === "blueprint") setActiveSection("blueprint");
    if (section === "chapters") setActiveSection("chapters");

    switch (action) {
      case "continue-writing":
        setActiveSection("chapters");
        break;
      case "generate-full-book":
        setActiveSection("chapters");
        setTimeout(() => {
          if (!quota?.isOverTokenLimit) {
            engine.generateFullBook((nextSection) => setActiveSection(nextSection as SectionId));
          } else {
      ...
      case "open-quality":
        setActiveSection("chapters");
        setShowCoach(true);
        window.dispatchEvent(new CustomEvent("nexora-open-ai-quality"));
        break;
      case "open-cover-studio":
        setShowCover(true);
        break;
      case "open-export":
        setShowPublish(true);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const openAction = sessionStorage.getItem("nexora-open-action");

      if (openSection) sessionStorage.removeItem("nexora-open-section");
      if (openAction) sessionStorage.removeItem("nexora-open-action");

      const applySection = () => {
        applyLaunchIntent(openAction, openSection);
      };
