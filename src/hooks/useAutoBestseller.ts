...
export function useAutoBestseller() {
  ...
  const progressLogRef = useRef<any[]>([]);
  ...
  const start = useCallback(async (input: AutoBestsellerInput, batchId?: string): Promise<AutoBestsellerResult | null> => {
    ...
    const persistSnapshot = (status: string, patch?: Partial<PersistedRunSnapshot>) => {
      ...
      writeRunSnapshot(runId, {
        input,
        progress: [...progressLogRef.current],
        result: lastResult,
        error: null,
        status,
    ...
    const queueProgressSync = () => {
      ...
      if (!runId || syncTimerRef.current !== null) return;
    ...
    const { abort, promise } = runAutoBestsellerStream(input, {
      onStage: (s) => {
        ...
        if (s.status === "running") {
          const labelMap: Record<string, string> = {
            titles: "Generating title strategy…",
            market: "Validating market opportunity…",
            blueprint: "Building launch blueprint…",
            gono: "Validating book concept…",
            chapters: "Preparing project workspace…",
            aggregate: "Finalizing launch-ready project…",
          };
          setState((prev) => ({
            ...prev,
        ...
        progressLogRef.current.push({ type: "stage", ...s, ts: Date.now() });
        queueProgressSync();
      },
      onRetry: (r) => {
        retryCountRef.current += 1;
        setState((prev) => ({ ...prev, retries: [...prev.retries, r] }));
        progressLogRef.current.push({ type: "retry", ...r, ts: Date.now() });
        queueProgressSync();
      },
      onBook: (b: BookEvent) => {
        ...
        progressLogRef.current.push({ type: "book", ...b, ts: Date.now() });
        queueProgressSync();
      },
      onChapter: (c) => {
        ...
        setState((prev) => {
          ...
          const liveWordCount = String(c.content || "").trim().split(/\s+/).filter(Boolean).length;
          const liveLabel =
            c.phase === "done"
              ? `Chapter ${c.index + 1}/${c.total} ready · ${liveWordCount} words`
              : c.phase === "refining"
                ? `Refining Chapter ${c.index + 1}/${c.total} · ${liveWordCount} words`
                : `Writing Chapter ${c.index + 1}/${c.total} · ${liveWordCount} words`;
        ...
        progressLogRef.current.push({ type: "chapter", ...c, ts: Date.now() });
        queueProgressSync();
      },
      onDone: (r) => {
        clearSyncTimer();
        ...
        setState((prev) => ({
          ...
          result: r,
          isRunning: false,
          liveBook: { ...prev.liveBook, currentStageLabel: "Project ready — open in Scriptora Studio" },
        }));
        progressLogRef.current.push({ type: "done", ts: Date.now() });
      ...
      onError: (msg) => {
        ...
        progressLogRef.current.push({ type: "error", message: msg, ts: Date.now() });
  ...
  const cancel = useCallback(async () => {
    const currentRunId = runIdRef.current || state.runId;
    ...
    if (currentRunId) {
      writeRunSnapshot(currentRunId, {
        ...
        status: "cancelled",
        retryCount: retryCountRef.current,
        finalScore: state.result?.finalScore ?? null,
        marketScore: state.result?.marketScore ?? null,
        partialProjectId: partialProjectIdRef.current,
        updatedAt: Date.now(),
      });

      void updateRunRow(currentRunId, {
        status: "cancelled",
        error: "Cancelled by user",
        progress: [...progressLogRef.current],
        retry_count: retryCountRef.current,
    ...
    setState((prev) => ({
      ...
      isRunning: false,
      error: "Cancelled by user",
      liveBook: { ...prev.liveBook, currentStageLabel: "Generation cancelled" },
    }));
  }, [state.runId, state.result, stopPolling]);
  ...
  const applyProgressLog = useCallback((row: any) => {
    ...
    const isTerminal = ["ready_for_kdp", "needs_revision", "failed", "cancelled"].includes(row?.status);
    setState((prev) => ({
      ...prev,
