      ...
      try {
        ...
        draft = await writeChapter(input, title, subtitle, blueprint, i, {
          ...
          await send("chapter", {
            ...
            title: outline.title,
            content: accumulated,
            currentWords: String(accumulated || "").trim().split(/\s+/).filter(Boolean).length,
            targetWords: adjustedTarget,
            statusText: "Writing chapter body",
          });
        });
      } catch (e) {
...
Length: ~800 words. Self-contained, no preamble. Plain prose, no JSON.`;
    ...
    try {
      ...
      console.log(`[AutoBestseller] Moving to next chapter after ${outline.title}`);
    ...
    await send("chapter", {
      ...
      voiceConfidence: refined.rewriteConfidence,
      content: refined.finalText,
      currentWords: countWords(refined.finalText),
      targetWords: adjustedTarget,
