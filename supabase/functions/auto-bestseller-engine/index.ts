...
Return ONLY this internal subchapter prose.`;
      ...
      if (onDelta) {
        sectionText = await callDeepSeekStream(system, sectionPrompt, 0.82, sectionMaxTokens, async (_chunk, acc) => {
          ...
          await onDelta(combined, {
            ...
            statusText: `Writing subchapter ${sectionIndex + 1}/${sectionCount}`,
    ...
    const minChapterWords = Math.max(900, Math.floor(ctx.targetWords * 0.82));
    const currentWords = countWords(clean);

    if (currentWords < minChapterWords) {
      const remainingWords = Math.max(280, ctx.targetWords - currentWords);
      const extensionMaxTokens = Math.min(1800, Math.ceil(remainingWords * 2.2));

      const extensionPrompt = `${user}

CONTINUATION MODE:
The chapter is already in progress but is still too short.
Continue EXACTLY from the end of the existing text.
Do NOT restart the chapter.
Do NOT repeat previous sections.
Do NOT add a chapter title.
Do NOT summarize what has already been said.
...
TASK:
Write the NEXT final section of this same chapter.
Target approximately ${remainingWords} additional words.
Open with a markdown ## heading only if it helps structure.
Deepen the argument, add concrete specifics, and close the chapter with a satisfying ending.`;

      let extension = "";
      if (onDelta) {
        extension = await callDeepSeekStream(system, extensionPrompt, 0.82, extensionMaxTokens, async (_chunk, acc) => {
          await onDelta((clean + "\n\n" + acc).trim());
        }, "deepseek-chat", `auto_bestseller_chapter_${chapterIndex + 1}_extension`);
      } else {
        extension = await callDeepSeek(system, extensionPrompt, false, 0.82, extensionMaxTokens);
      }
...
async function runPipeline(
  ...
  // STAGE 5: launch-ready project package (no long-form chapter generation here)
  await checkpoint();
  await send("stage", { stage: "chapters", status: "done", label: "Project structure prepared — writing continues in Scriptora Studio" });

  const marketScore = winner.marketScore;
  const conceptScore =
    conceptVerdict?.verdict === "GO"
      ? 8.4
      : conceptVerdict?.verdict === "MAYBE"
        ? 7.2
        : 4.5;

  const finalScore = Math.round((((marketScore || 0) + conceptScore) / 2) * 10) / 10;

  const status: OrchestratorOutput["status"] =
    marketScore >= 7 && conceptVerdict?.verdict === "GO"
      ? "ready_for_kdp"
      : marketScore >= 5 || conceptVerdict?.verdict === "MAYBE"
        ? "needs_revision"
        : "failed";
  ...
  const result: OrchestratorOutput = {
    ...
    pipeline: {
      mode: "launch_engine",
      titleCandidates: allScored.map((c) => ({
        title: c.title,
        subtitle: c.subtitle,
      ...
      conceptVerdict,
      conceptRetries: retries,
      chapterCount: blueprint?.chapterOutlines?.length || 0,
      avgVoiceConfidence: 0,
      nextStep: "open_in_scriptora_studio",
      launchReady: true,
    },
  };

  await checkpoint();
  await send("stage", { stage: "aggregate", status: "running", label: "Finalizing launch-ready project" });
  await send("stage", { stage: "aggregate", status: "done", label: `Project ready • status ${status}` });
  return result;
}
...
async function isRunCancelled(runId: string): Promise<boolean> {
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/auto_bestseller_runs?id=eq.${runId}&select=status`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "apikey": SERVICE_ROLE,
        ...
        const pipelineWork = (async () => {
          try {
            const result = await runPipeline(
              ...
              send,
              async () => {
                await throwIfCancelled(runId ?? undefined);
              },
            );
            ...
            if (code === "RUN_CANCELLED") {
              await send("cancelled", { message: "Cancelled by user" });
              if (runId) {
                await persistProgressToDb(runId, [...progressLog], {
                  status: "cancelled",
